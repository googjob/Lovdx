...
function houDuan(req, res) {
  ...
  // 然后判断，如果来的网格请求是GET方法，那么直接end返回文字数据。
  if (req.method === "GET") {
    // 判断是否有数据文件
    if (fs.existsSync("wenZi.txt")) {
      ...
      res.edn(JSON.stringify({ data: wenZi }));
    } else {
      // 没有则返回空对象
      res.end("{}");
    }
    ...
    // 再调用fs的写文件函数把数据写入文件。
    // decodeURI 函数解析中文
    // substring的作用可看：https://www.runoob.com/jsref/jsref-substing.html
